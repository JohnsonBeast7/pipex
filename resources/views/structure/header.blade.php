    ...
    <div class="min-h-16 flex flex-row justify-between items-center w-full max-w-[1324px] px-6 m-auto">
        <div class="flex flex-col gap-1">
            <a href="{{ route('home') }}" class="w-full @guest md:max-w-[185px] max-w-fit @endguest @auth max-w-fit @endauth flex flex-row items-center">
                <h1 class="text-white md:text-5xl text-4xl font-medium">Pipe</h1>
                <span class="text-sky-500 font-bold md:text-6xl text-5xl">X</span>
            </a>  
            @if(Route::is('home'))
        ...
        @if(!Route::is(['login', 'register', 'profileEdit']))      
            @guest
                <div class="flex flex-col items-center gap-1">
                    <p class="text-white md:text-base text-sm font-normal"><a href="{{ route('login') }}" class="text-sky-500 font-medium hover:font-semibold transition duration-500">Entrar</a> na minha conta.</p>
                    <p class="text-white md:text-base text-sm font-normal">Não tem uma? <a href="{{ route('register') }}" class="text-sky-500 font-medium hover:font-semibold transition duration-500">Crie</a> agora!</p>
                </div>
            @endguest
